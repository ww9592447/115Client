from PyQt5.Qt import QFrame, QCursor, QApplication, Qt, QPushButton, \
    QPainter, QPainterPath, QColor, QObject, QEvent, QRect
...
class ScrollArea(QFrame):
    ...
    def __init__(self, parent, visualcontents: QFrame = None, scrollcontents: QFrame = None, scrollsize: int = 14,
                 singlestep: int = 20, pagestep: int = 60, interval: int = 5, arrow: bool = True) -> None:
        super().__init__(parent=parent)
        # 滾動條大小
        self.scrollsize: int = scrollsize
        # 滾動條箭頭移動距離
        self.singlestep: int = singlestep
        # 滾條條背景點擊移動距離
        self.pagestep: int = pagestep
        ...
        # 設置直滾動條容器
        self.verticalcontents: ScrolContainer = ScrolContainer(self, 1, singlestep, interval, arrow)
        # 設置橫滾動條容器
        self.hrizontalcontents: ScrolContainer = ScrolContainer(self, 2, singlestep, interval, arrow)

    # 添加新的內容窗口
    def setwidget(self, scrollcontents: QFrame) -> None:
        # 查看是否有舊的內窗口
    ...
    # 手動設置直滾動條是否能夠顯示
    def setvertical(self, status: bool) -> None:
        # 設置垂直滾動條狀態
    ...
    # 手動設置橫滾動條是否能夠顯示
    def sethrizontal(self, status: bool) -> None:
        # 設置橫滾動條狀態
    ...
    def setvalue(self, orientation: int, direction: bool, move: int) -> None:
        # 獲取內容容器yx座標
    ...
    # 設定內容窗口移動
    def scrollcontentsmove(self, x: int, y: int) -> None:
        # 獲取內容窗口 x 移動距離
    ...
    def setdistance(self) -> None:
        ...
        if -self.scrollcontents.y() > 0:
            ...
            # 內容容器 - 底部
            if (y := self.visualcontents.height() - bottom) > 0\
                    and self.scrollcontents.height() >= self.visualcontents.height():
        ...
        if -self.scrollcontents.x() > 0:
            ...
            # 內容容器 - 底部
            if (x := self.visualcontents.width() - bottom) > 0 \
                    and self.scrollcontents.width() >= self.visualcontents.width():
...
class ScrolContainer(QFrame):
    def __init__(self, parent: ScrollArea, orientation: int, singlestep: int, interval: int, arrow: bool) -> None:
        ...
        # 步進距離
        self.scrolljump: int = 0
        # 箭頭步進距離 背景點擊步進距離
        self.singlestep: int = singlestep
        # 滾動上限
        self.limit: int = 0
        # 滑塊大小
        self.slidersize: int = 0
        # 滑塊容器 滑動全部空間
        self.scrollbackground: Scroll = Scroll(self, orientation)
        # 設定滑塊
        self.slider: Slider = self.scrollbackground.slider
        # 是否顯示箭頭
        if arrow:
            # 距離邊緣距離
    ...
    def setGeometry(self, *rect: Union[QRect, int]) -> None:
        ...
        # 可視範圍
        viewport = self.parent().visualcontents.height() if self.orientation == 1\
            else self.parent().visualcontents.width()
        # 總範圍
        content = self.parent().scrollcontents.height() if self.orientation == 1\
            else self.parent().scrollcontents.width()
        if content == 0:
            return
        # 可視範圍/總範圍 = 總共多少頁
    ...
    def setvalue(self, value: int) -> None:
        ...
        if self.parent().scrollcontents and self.limit > 0:
            ...
            if self.scrolljump != 0:
                # 獲取滑塊移動數據
                _move = (0, -int(yx() / self.scrolljump)) if self.orientation == 1 \
                    else (-int(yx() / self.scrolljump), 0)
                # 設定滑塊移動
...
class Scroll(QPushButton):
    def __init__(self, parent: ScrolContainer, orientation: int) -> None:
        super().__init__(parent)
        # 背景方向
        self.orientation: int = orientation
        # 滑塊
